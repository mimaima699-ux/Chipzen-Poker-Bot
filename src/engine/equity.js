...
//
// Based on poker/server/src/ai/equity.js, adapted for the Chipzen container:
// the platform enforces a hard decision timeout (500ms for uploaded bots),
// so every Monte Carlo loop takes an optional `deadline` (absolute Date.now()
// timestamp) and stops early — never below a floor of 60 samples — instead of
// blindly finishing its iteration count.
//
// `monteCarloEquity` is the workhorse: it repeatedly deals out the rest of the
// board and each opponent's hole cards, then grades the hands with the exact
...
import { createDeck, shuffle, SUITS, RANKS } from './deck.js'

const SUIT_INDEX = { c: 0, d: 1, h: 2, s: 3 }
const cardKey = (c) => c.rank * 4 + SUIT_INDEX[c.suit]

// Stop sampling once the wall clock passes `deadline`, but always take at
// least `floor` samples so a slow cold start can't zero out the estimate.
function pastDeadline(deadline, done, floor) {
  if (done < floor) return false
  if (deadline === undefined) return false
  if ((done & 15) !== 0) return false // only touch the clock every 16 samples
...
export function monteCarloEquity(hole, community, { opponents = 1, iterations = 500, rng = Math.random, deadline } = {}) {
  ...
  for (; done < iterations; done++) {
    if (pastDeadline(deadline, done, 60)) break
...
export function preflopEquity(c1, c2, opponents, deadline) {
  ...
  if (!preflopCache.has(key)) {
    const hole =
      high === low
        ? [{ rank: high, suit: 's' }, { rank: low, suit: 'h' }] // pocket pair
        : suited
          ? [{ rank: high, suit: 's' }, { rank: low, suit: 's' }]
          : [{ rank: high, suit: 's' }, { rank: low, suit: 'h' }]
    preflopCache.set(key, monteCarloEquity(hole, [], { opponents, iterations: 400, deadline }))
  }
  return preflopCache.get(key)
}

// One entry point for the AI: real equity against `opponents` remaining players.
export function equityFor(hole, community, opponents, deadline) {
  if (!hole || hole.length < 2) return 0
  if (community.length === 0) return preflopEquity(hole[0], hole[1], opponents, deadline)
  return monteCarloEquity(hole, community, { opponents, deadline })
}
...
export function equityVsRanges(hole, community, ranges, { iterations = 400, rng = Math.random, deadline } = {}) {
  if (!hole || hole.length < 2) return 0
  const seen = new Set([...hole, ...community].map(cardKey))
  let win = 0
  let tie = 0
